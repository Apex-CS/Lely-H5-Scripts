/**
 *  Purpose of the H5 script: On order position entry in MMS101, this script will check if an item is blocked
...
var MMS101B_OrderEntryItemCheck = /** @class */ (function () {
    function MMS101B_OrderEntryItemCheck(args) {
        this.TRTP = "";
        this.OrdType = "";
        this.ToWarehouse = "";
        this.TTYP = "";
    ...
    */
    MMS101B_OrderEntryItemCheck.Init = function (args) {
        new MMS101B_OrderEntryItemCheck(args).run();
        //this.log.Info("In init");
    };
    MMS101B_OrderEntryItemCheck.prototype.run = function () {
        ...
        var DoNum = this.controller.GetValue("MRTRNR"); //get the Co number from B panel  
        this.callAPI1(DoNum);
        this.attachEvents(this.controller);
    };
    MMS101B_OrderEntryItemCheck.prototype.callAPI = function (Company, FILE, TXID, langCode, Itemno) {
    ...
    MMS101B_OrderEntryItemCheck.prototype.callAPI1 = function (DoNum) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        this.log.Info("IfcallAPI1");
                        return [4 /*yield*/, this.MMS100MIGetHead(DoNum)];
                    case 1:
                        _a.sent();
                        return [4 /*yield*/, this.CRS200MILstOrderType()];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, this.MMS005MIGetWarehouse()];
                    case 3:
                        _a.sent();
                        return [2 /*return*/];
                }
    ...
    MMS101B_OrderEntryItemCheck.prototype.MMS100MIGetHead = function (DoNum) {
        this.log.Info("callMIfirst");
        var _this = this;
        return new Promise(function (resolve) {
            var request = {
                url: '/M3/m3api-rest/v2/execute/MMS100MI/GetHead/',
                method: "GET",
                record: {
                    TRNR: DoNum
                }
            ...
            _this.ionApiService.execute(request).then(function (response) {
                ...
                _this.OrdType = result[0].TRTP;
                _this.ToWarehouse = result[0].TWLO;
                _this.log.Info("MMS100MI Response " + result[0].TRTP);
                _this.log.Info("MMS100MI Response " + result[0].TWLO);
                if (null != _this.OrdType) {
                    resolve(_this.OrdType);
                }
                else {
                    resolve(null);
                }
    ...
    ;
    MMS101B_OrderEntryItemCheck.prototype.CRS200MILstOrderType = function () {
        this.log.Info("callMIsecond");
        var _this = this;
        return new Promise(function (resolve) {
            var request = {
                url: '/M3/m3api-rest/v2/execute/CRS200MI/LstOrderType/',
                method: "GET",
                record: {
                    TRTP: _this.OrdType
                }
            ...
            _this.ionApiService.execute(request).then(function (response) {
                var result = response.data.results[0].records;
                _this.TTYP = result[0].TTYP;
                _this.log.Info("CRS200MI Response " + result[0].TTYP);
                if (null != _this.TTYP) {
                    resolve(_this.TTYP);
                }
                else {
    ...
    ;
    MMS101B_OrderEntryItemCheck.prototype.MMS005MIGetWarehouse = function () {
        this.log.Info("callMIthird");
        var _this = this;
        return new Promise(function (resolve) {
            var request = {
                url: '/M3/m3api-rest/v2/execute/MMS005MI/GetWarehouse/',
                method: "GET",
                record: {
                    WHLO: _this.ToWarehouse
                }
            };
            _this.ionApiService.execute(request).then(function (response) {
                var result = response.data.results[0].records;
                _this.CSCD = result[0].CSCD;
                _this.log.Info("MMS005MI Response " + result[0].CSCD);
                if (null != _this.CSCD) {
                    resolve(_this.CSCD);
                }
                else {
    ...
    ;
    MMS101B_OrderEntryItemCheck.prototype.CRS980MIgetTextID = function (file, Company, Item) {
        var _this = this;
    ...
    MMS101B_OrderEntryItemCheck.prototype.CRS980MISelTextBlock = function (Company, txid, langCode) {
                        ...
                        if (response.data.results != null && response.data.results != undefined) {
                            ...
                            for (var i = 0; i < result.length; i++) {
                                ...
                                if (i == 3) {
                                    ...
                                    _this.log.Info("scopetx60" + _this.Scope);
                                }
                                ...
                                if (result[i].TX60.includes(langCode) && !result[i].TX60.includes("<scope>")) {
                                    ...
                                    _this.log.Info("lanothertx60" + _this.lanOther);
                                }
                            }
    ...
    MMS101B_OrderEntryItemCheck.prototype.attachEvents = function (controller) {
        var _this_1 = this;
        this.unsubscribeRequesting = controller.Requesting.On(function (e) {
            //Get enter event		
            if (e.commandType === "KEY" && e.commandValue === "ENTER" && _this_1.controller.GetPanelName() == "MMA101BC") {
                if (_this_1.firstTime) {
                    var Itemno = ScriptUtil.GetFieldValue("WRITNO");
                ...
                _this_1.log.Info("firsttime" + "" + _this_1.firstTime);
                _this_1.log.Info("scope" + _this_1.Scope);
                _this_1.log.Info("country" + _this_1.CSCD);
                _this_1.log.Info("lanother" + _this_1.lanOther);
                _this_1.log.Info("lngcode" + _this_1.langCode);
                _this_1.log.Info("Check 1" + _this_1.Scope.includes(_this_1.CSCD));
                _this_1.log.Info("Check 2" + _this_1.validDate);
                if (_this_1.ToWarehouse === "") {
                    _this_1.log.Info("If0");
                    _this_1.controller.ShowMessage("Destination Warehouse is not set");
                    _this_1.firstTime = true;
                    e.cancel = true;
                    _this_1.detachEvents();
                }
                else if (_this_1.TTYP === "40" || _this_1.TTYP === "41" || _this_1.TTYP === "") {
                    _this_1.log.Info("If1");
                    _this_1.controller.ShowMessage("DO type not allowed");
                    _this_1.firstTime = true;
                    e.cancel = true;
                    _this_1.detachEvents();
                }
    ...
    MMS101B_OrderEntryItemCheck.prototype.detachEvents = function () {
        this.unsubscribeRequesting();
    };
    return MMS101B_OrderEntryItemCheck;
}());
//# sourceMappingURL=MMS101B_OrderEntryItemCheck.js.map